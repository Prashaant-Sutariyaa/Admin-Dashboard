...
const SentinelBatchUploadDialog = ({ open, onOpenChange }: Props) => {
                ...
                allDepartments.filter(
                    (d: Department) => {
                        if (currentDepartment === "DataOps") {
                            return ["DataOps", "DBR"].includes(d.name);
                        }
                        return (
    ...
    const isDisabled = !campaignId || !segmentId || !departmentId || !file;
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent className="max-w-5xl p-0 overflow-hidden">
                ...
                <DialogFooter className="px-6 py-4 border-t bg-muted/20">
                    <Button variant="lighterror" onClick={() => onOpenChange(false)}>
                        Cancel
                    </Button>

                    <Button variant="lightprimary" disabled={isDisabled || loading} onClick={handleUpload}>
                        {loading ? "Loading..." : "Upload Batch"}
                    </Button>
                </DialogFooter>
